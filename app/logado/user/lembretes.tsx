...
const Lembretes = () => {
  ...
  useEffect(() => {
    const carregarLembretes = async () => {
      try {
        if (user?.id) {
          const salvos = await AsyncStorage.getItem(`@lembretes_${user.id}`);
          if (salvos) {
  ...
  const calcularProximaDose = (
    ...
    const proximaDose = new Date(
      horarioBase.getTime() +
        (intervalosPassados + 1) * intervalo * 60 * 60 * 1000
    );
  ...
  const renderDetalhesLembrete = (lembrete: LembreteSalvo) => {
    ...
    switch (lembrete.tipo) {
      case "Exames":
        return (
          <>
            <Text style={styles.lembreteDetalhe}>
              Exame: {lembrete.nomeExame}
            </Text>
            <Text style={styles.lembreteDetalhe}>
              Data: {formatDate(lembrete.data)}
            </Text>
            <Text style={styles.lembreteDetalhe}>
              Horário: {formatTime(lembrete.horario)}
            </Text>
          </>
        );
      case "Medicamentos":
        const diasRestantesText =
          diasRestantes === "Finalizado" ? (
            <Text style={[styles.lembreteDetalhe, styles.lembreteFinalizado]}>
              Duração: Finalizado
            </Text>
          ) : (
            <Text style={styles.lembreteDetalhe}>
              Duração: {lembrete.duracao} dias - restam {diasRestantes}
            </Text>
          );
        ...
        return (
          <>
            ...
            {diasRestantesText}
            <Text style={styles.lembreteDetalhe}>
              Intervalo: {lembrete.intervalo} horas
            </Text>
            <Text style={styles.lembreteDetalhe}>
      ...
      case "Hidratação":
        return (
          <>
            <Text style={styles.lembreteDetalhe}>
              Quantidade: {lembrete.quantidade} ml
            </Text>
            <Text style={styles.lembreteDetalhe}>
              Intervalo: {lembrete.intervalo} horas
            </Text>
            <Text style={styles.lembreteDetalhe}>
              Próxima hidratação: {proximaDose}
            </Text>
          </>
        );
      case "Pressão":
        ...
        <View style={styles.avisoView}>
          {lembretes.length === 0 ? (
            <Text style={styles.aviso}>
              Você não tem nenhum lembrete adicionado
            </Text>
          ) : (
              ...
              {lembretes.map((lembrete) => (
                <View key={lembrete.id} style={styles.lembreteItem}>
                  <View style={styles.lembreteHeader}>
                    ...
                    <TouchableOpacity
                      onPress={() => removerLembrete(lembrete.id)}
                    >
                      <FontAwesome5
                        name="trash"
                        size={20}
                ...
                <>
                  ...
                  <View style={styles.section}>
                    ...
                    <TextInput
                      ...
                      value={duracao}
                      onChangeText={(text) =>
                        handleNumberChange(text, setDuracao)
                      }
                    />
                  </View>
                  ...
                  <View style={styles.section}>
                    ...
                    <TextInput
                      ...
                      value={intervalo}
                      onChangeText={(text) =>
                        handleNumberChange(text, setIntervalo)
                      }
                    />
                  </View>
                </>
                ...
                <>
                  <View style={styles.section}>
                    ...
                    <TextInput
                      ...
                      value={quantidade}
                      onChangeText={(text) =>
                        handleNumberChange(text, setQuantidade)
                      }
                    />
                  </View>
